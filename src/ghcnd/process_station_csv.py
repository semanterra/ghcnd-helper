...
def process_station_csv(tar_info, buff, output_dict):
    ...
    station_df = station_df.with_columns(nulled_df)
    ...
    for attr in ['MEASURE', 'QUALITY', 'SOURCE']:
        for obvalue in station_obvalues:
            col = obvalue + '_' + attr
            x = (station_df[col].value_counts()
                 .rename({col:'VALUE', 'count':'COUNT'})
                 .filter(pl.col('VALUE').is_not_null()))
            if x.height:
                x.insert_column(0, pl.lit(station).alias('STATION'))
                x.insert_column(1, pl.lit(obvalue).alias('OBVALUE'))
                x.insert_column(2, pl.lit(attr).alias('ATTR'))

                output_dict['stations_attr_use'].vstack(x, in_place=True)
'''
    # TODO
